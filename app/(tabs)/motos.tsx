...
import { StyleSheet, FlatList, ActivityIndicator, TouchableOpacity, RefreshControl, Alert, View } from 'react-native';
...
export default function MotosScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      console.warn('Error loading motos', err);
      Alert.alert(t('error_label'), err?.message ?? t('fetch_fail'));
    } finally {
      setLoading(false);
    }
  }, [t]);
